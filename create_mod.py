...
supported_languages = []
all_languages = ["br", "de", "en", "es", "fr", "hi", "it", "jp", "ko", "ms", "mx", "pl", "ru", "tr", "tw", "vi", "zh"]
languageBugFixxed = 0 # currently if you play a mod, the game loads the modded-key-value in the language folder of that mod folder structure...
    # ...even tho it should always use english (or the only modded-strings that exists?) if that folder doesn't exist, that was supposed to be fixed, but it's not
    # therefore you still need to create all language folders and put the english file in it. And yes, they have to be the same file for every player meaning
    # multilanguage support is NOT even possible cause the game checks if ALL the files are the same, so all modded-strings must have the same checksum...dumb right?
...
def create_file_structure():
    ...
    if languageBugFixxed:
        ...
        for lang_shortcut in all_languages:
            lang_folders = (Path(__file__).parent / "finished_mods" / "Better Land Warfare (Official Data Mod)" / "resources" / f"{lang_shortcut}" / "strings" / "key-value").resolve()
            lang_folders.mkdir(parents = True, exist_ok=True)
            storage.languageFolders.append(lang_folders)
...
def creating_lang_files():
    ...
    with open(languageFilePath, "r", encoding="utf-8") as langfile:
        if languageBugFixxed:
            ...
            for langfolder in storage.languageFolders:
                newFile = langfolder / "key-value-modded-strings-utf8.txt"
                shutil.copy(languageFilePath, langfolder)
